...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% calfun.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE OCALFUN (N,X,F)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION X(*)
      COMMON FMODE, OFMODE
      IF (OFMODE .EQ. 9999) THEN
         CALL MOOBobyqaEvalFunc(N, X, F);
      ELSE IF (OFMODE .EQ. 8888) THEN
         CALL KRIBobyqaEvalFunc(N, X, F);
      ELSE IF (OFMODE .EQ. 7777) THEN
         CALL BobyqaEvalFunc(N, X, F);
      ELSE IF (OFMODE .EQ. 6666) THEN
         CALL EvaluateFunctionMM(N, X, F);
      ELSE IF (OFMODE .EQ. 5555) THEN
         CALL EvaluateFunctionSM(N, X, F);
      ELSE IF (OFMODE .EQ. 4444) THEN
         CALL PKRIBobyqaEvalFunc(N, X, F);
      ELSE IF (OFMODE .EQ. 3333) THEN
         CALL OUU1EvalFunc(N, X, F);
      ELSE IF (OFMODE .EQ. 2222) THEN
         CALL OUU2EvalFunc(N, X, F);
      ELSE IF (OFMODE .EQ. 2223) THEN
         CALL OUU2EvalFunc2(N, X, F);
      ELSE IF (OFMODE .EQ. 1111) THEN
         CALL OUUEvalFunc(N, X, F);
      ELSE IF (OFMODE .EQ. 1112) THEN
         CALL OUUEvalFunc2(N, X, F);
      ELSE
         PRINT *, 'PBOBYQA CALFUN : NO FUNCTION PROVIDED.'
         PRINT *, 'CODE = ', OFMODE
         F = 0.0
      END IF
      RETURN
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% bobyqa.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE OBOBYQA (N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,
     1  MAXFUN,W)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION X(*),XL(*),XU(*),W(*)
      COMMON FMODE, OFMODE
...
C
   ...
   10     FORMAT (/4X,'Return from BOBYQA because NPT is not in',
     1      ' the required interval')
          PRINT *, 'NPT = ', NPT
          PRINT *, 'NPT should be >= ', N+2
          PRINT *, 'NPT should be <= ', ((N+2)*NP/2)
          GO TO 40
      END IF
C
...
C
      CALL OBOBYQB (N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,MAXFUN,W(IXB),
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% bobyqb.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE OBOBYQB (N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,
...
C
      CALL OPRELIM (N,NPT,X,XL,XU,RHOBEG,IPRINT,MAXFUN,XBASE,XPT,
...
C
   60 CALL OTRSBOX (N,NPT,XPT,XOPT,GOPT,HQ,PQ,SL,SU,DELTA,XNEW,D,
...
C
  190 NFSAV=NF
      KBASE=KOPT
      CALL ORESCUE (N,NPT,XL,XU,IPRINT,MAXFUN,XBASE,XPT,FVAL,
...
C
  210 CALL OALTMOV (N,NPT,XPT,XOPT,BMAT,ZMAT,NDIM,SL,SU,KOPT,
...
C
      CALL OUPDATEB(N,NPT,BMAT,ZMAT,NDIM,VLAG,BETA,DENOM,KNEW,W)
      IH=0
      PQOLD=PQ(KNEW)
      PQ(KNEW)=ZERO
      DO 470 I=1,N
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% altmov.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE OALTMOV (N,NPT,XPT,XOPT,BMAT,ZMAT,NDIM,SL,SU,KOPT,
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% prelim.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE OPRELIM (N,NPT,X,XL,XU,RHOBEG,IPRINT,MAXFUN,XBASE,
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% rescue.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE ORESCUE (N,NPT,XL,XU,IPRINT,MAXFUN,XBASE,XPT,
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% trsbox.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE OTRSBOX (N,NPT,XPT,XOPT,GOPT,HQ,PQ,SL,SU,DELTA,
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% update.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE OUPDATEB(N,NPT,BMAT,ZMAT,NDIM,VLAG,BETA,DENOM,
     1  KNEW,W)
      IMPLICIT REAL*8 (A-H,O-Z)
