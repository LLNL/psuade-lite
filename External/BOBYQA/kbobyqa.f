...
C     - create KFMODE common variable to choose between MOO and Bobyqa
C **********************************************************************
C
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% calfun.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE KCALFUN (N,X,F)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION X(*)
      COMMON FMODE, OFMODE, KFMODE
      IF (KFMODE .EQ. 9999) THEN
         CALL MOOBobyqaEvalFunc(N, X, F);
      ELSE IF (KFMODE .EQ. 8888) THEN
         CALL KRIBobyqaEvalFunc(N, X, F);
      ELSE IF (KFMODE .EQ. 7777) THEN
         CALL BobyqaEvalFunc(N, X, F);
      ELSE IF (KFMODE .EQ. 6666) THEN
         CALL EvaluateFunctionMM(N, X, F);
      ELSE IF (KFMODE .EQ. 5555) THEN
         CALL EvaluateFunctionSM(N, X, F);
      ELSE IF (KFMODE .EQ. 4444) THEN
         CALL PKRIBobyqaEvalFunc(N, X, F);
      ELSE IF (KFMODE .EQ. 3333) THEN
         CALL OUUEvalFunc(N, X, F);
      ELSE IF (KFMODE .EQ. 2222) THEN
         CALL OUU2EvalFunc(N, X, F);
      ELSE IF (KFMODE .EQ. 1111) THEN
         CALL OUU2EvalFunc2(N, X, F);
      ELSE
         PRINT *, 'BOBYQA KCALFUN : NO FUNCTION PROVIDED.'
         PRINT *, 'CODE = ', KFMODE
         F = 0.0
      END IF
      RETURN
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% bobyqa.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE KBOBYQA (N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,
     1  MAXFUN,W)
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION X(*),XL(*),XU(*),W(*)
      COMMON FMODE, OFMODE, KFMODE
...
C
      CALL KBOBYQB (N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,MAXFUN,W(IXB),
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% bobyqb.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE KBOBYQB (N,NPT,X,XL,XU,RHOBEG,RHOEND,IPRINT,
...
C
      CALL KPRELIM (N,NPT,X,XL,XU,RHOBEG,IPRINT,MAXFUN,XBASE,XPT,
...
C
   60 CALL KTRSBOX (N,NPT,XPT,XOPT,GOPT,HQ,PQ,SL,SU,DELTA,XNEW,D,
...
C
  190 NFSAV=NF
      KBASE=KOPT
      CALL KRESCUE (N,NPT,XL,XU,IPRINT,MAXFUN,XBASE,XPT,FVAL,
...
C
  210 CALL KALTMOV (N,NPT,XPT,XOPT,BMAT,ZMAT,NDIM,SL,SU,KOPT,
...
C
      CALL KUPDATEB(N,NPT,BMAT,ZMAT,NDIM,VLAG,BETA,DENOM,KNEW,W)
      IH=0
      PQOLD=PQ(KNEW)
      PQ(KNEW)=ZERO
      DO 470 I=1,N
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% altmov.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE KALTMOV (N,NPT,XPT,XOPT,BMAT,ZMAT,NDIM,SL,SU,KOPT,
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% prelim.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE KPRELIM (N,NPT,X,XL,XU,RHOBEG,IPRINT,MAXFUN,XBASE,
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% rescue.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE KRESCUE (N,NPT,XL,XU,IPRINT,MAXFUN,XBASE,XPT,
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% trsbox.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE KTRSBOX (N,NPT,XPT,XOPT,GOPT,HQ,PQ,SL,SU,DELTA,
...
C%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% update.f %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      SUBROUTINE KUPDATEB(N,NPT,BMAT,ZMAT,NDIM,VLAG,BETA,DENOM,
     1  KNEW,W)
      IMPLICIT REAL*8 (A-H,O-Z)
