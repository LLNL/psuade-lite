...
C 9/20142. This file did not come with any copyright
...
C **********************************************************************
C Changes made: 
C    - CALFUN name has been changed to LCALFUN
C    - PRELIM name has been changed to LPRELIM
C **********************************************************************
...
C
      CALL LPRELIM (N,NPT,M,AMAT,B,X,RHOBEG,IPRINT,XBASE,XPT,FVAL,
...
C
  ...
  260     FORMAT (/4X,'Function number',I6,'    F =',1PD18.10,
     1      '    The corresponding XX is:'/(2X,5D15.6))
      END IF
...
C
      CALL UPDATELINCOA (N,NPT,XPT,BMAT,ZMAT,IDZ,NDIM,SP,STEP,KOPT,
     1  KNEW,PQW,W)
      IF (KNEW .EQ. 0) THEN
